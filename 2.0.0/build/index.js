...
kg/ks-timeline/2.0.0/base
kg/ks-timeline/2.0.0/background
kg/ks-timeline/2.0.0/toolbar
kg/ks-timeline/2.0.0/main-content
kg/ks-timeline/2.0.0/dd
kg/ks-timeline/2.0.0/config
kg/ks-timeline/2.0.0/control
kg/ks-timeline/2.0.0/index
...
KISSY.add('kg/ks-timeline/2.0.0/base', function(S){
  function Base(){
...
KISSY.add('kg/ks-timeline/2.0.0/background', function(S, Base, TT){
  var $ = S.all;
  var ATTRS = {
    html: {
      /*
...
KISSY.add('kg/ks-timeline/2.0.0/toolbar', function(S, Base){
  var $ = S.all;
...
KISSY.add('kg/ks-timeline/2.0.0/main-content', function(S, Base, TT, Anim){
  var $ = S.all;
  ...
  var o = {
    ...
    ,dragBuffer: function(buffer){
      ...
      self.dragBufferAnim = new Anim(self.get('timenav')[0],{
        ...
        duration: buffer.duration/2.0.00
        ,easing:'easeOutStrong'
        // ,frame: function(){
        //   if( self.isOverflow() ){
        //     return 0;
    ...
    ,renderInterval: function(onlyStyle){
      ...
      else if( yDistance < 2000 && yDistance >= 2.0.00){
        _y = 1;
        _m = 2;
      }
      else if( yDistance < 2.0.00 && yDistance >= 500){
        _y = 1;
        _m = 3;
      }
...
/**
  * 普通元素拖拽
  */
KISSY.add('kg/ks-timeline/2.0.0/dd', function(S, Base){
  var $ = S.all, D = S.DOM;
  ...
  var o = {
    ...
    ,_calculateBuffer: function(){
      ...
      retBuffer.duration = Math.abs( parseInt( retBuffer.v / retBuffer.a * 2.0.00, 10));
 ...
 */
KISSY.add('kg/ks-timeline/2.0.0/config', function(S){
  ...
  var Config = {
    ...
    //用户可配置
    ,scale: 200 //初始化时，一月的在标尺上的宽度 默认2.0.0px
    ,minRate: 3 //最大放大倍数
    ,maxRate: 3 //最小缩小倍数
...
KISSY.add('kg/ks-timeline/2.0.0/control', function(S, Base, BG, Toolbar, MainContent, DD, TrackConfig){
  var $ = S.all;
...
KISSY.add('kg/ks-timeline/2.0.0/index', function(S, Control){
  var $ = S.all;
  //class
  /**
    * @param{config} 
